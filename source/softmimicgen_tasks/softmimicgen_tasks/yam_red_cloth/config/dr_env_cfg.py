...
# The square source image repeats 6x6 across the 11.5-inch (0.2921 m) cloth, so each tile
# represents a 1.92-inch (4.87 cm) square of wiping-fabric weave. `project_uvw=False` samples
# authored mesh UVs, keeping the denser weave registered to the deforming surface.
_FABRIC_TEXTURES = [str(Path("~/Downloads/red_cloth.jpg").expanduser())]


@configclass
class YamRedClothDREnvCfg(YamRedClothEnvCfg):
    """YAM red-cloth folding with camera pose, lighting, and cloth appearance randomization."""
